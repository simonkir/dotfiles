...
(defun sk:set-face-fixed ()
  (interactive)
  (let (new-face height)
    (setq new-face (completing-read "new fixed-pitch face: " sk:fonts-fixed-pitch nil nil))
    (if (string= new-face "Inconsolata")
        (setq height 110)
      (setq height 100))
...
(defun sk:set-face-variable ()
  (interactive)
  (let (new-face height)
    (setq new-face (completing-read "new variable-pitch face: " sk:fonts-variable-pitch nil nil))
    (setq height 100)
